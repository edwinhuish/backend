<template>
  <div class="table-basic-vue frame-page h-panel h-panel-margin-0 w-1000">
    ...
    <div class="h-panel-body">
      ...
      <div class="float-box mb-10">
        <p-button glass="h-btn h-btn-primary" permission="course.subscribe.create" text="添加" @click="create()" />
      </div>
      <div class="float-box mb-10">
        <Table :loading="loading" :datas="data">
          ...
          <TableItem title="操作" :width="120">
            <template slot-scope="{ data }">
              <p-del-button permission="course.subscribe.delete" @click="remove(data)" />
            </template>
          </TableItem>
        </Table>
      </div>
