<template>
  <div class="table-basic-vue frame-page h-panel">
    ...
    <div class="h-panel-body">
      <div class="float-box mb-10">
        <Form>
          <Row :space="10">
            <Cell :width="6">
              <FormItem label="搜索">
                <input v-model="filter.key" type="text" placeholder="支持优惠码模糊搜索">
              </FormItem>
            </Cell>
            <Cell :width="6">
              <FormItem label="UID">
      ...
      <div class="float-box mb-10">
        <p-del-button text="批量删除" permission="promoCode.destroy.multi" @click="deleteSubmit()" />
        <p-button glass="h-btn h-btn-primary h-btn-s" icon="h-icon-plus" permission="promoCode.store" text="添加" @click="create()" />

        <p-button glass="h-btn h-btn-primary h-btn-s" permission="promoCode.import" text="批量导入" @click="showImportPage()" />

        <p-button glass="h-btn h-btn-primary h-btn-s" permission="promoCode.generator" text="批量生成" @click="showGeneratePage()" />
      </div>
      <div class="float-box mb-10">
        <Table ref="table" :loading="loading" :datas="datas" :checkbox="true" @sort="sortEvt">
          <TableItem prop="id" :sort="true" title="ID" :width="80" />
          <TableItem title="优惠码">
            <template slot-scope="{ data }">
              <copytext :copytext="data.code" />
            </template>
          </TableItem>
          <TableItem prop="invited_user_reward" :sort="true" title="抵扣" unit="元" :width="80" />
          <TableItem prop="invite_user_reward" :sort="true" title="奖励" unit="元" :width="80" />
          ...
          <TableItem prop="used_times" :sort="true" title="已使用" unit="次" :width="100" />
