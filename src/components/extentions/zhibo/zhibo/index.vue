      ...
      <div class="float-box mb-10">
        <Form>
          <Row :space="10">
            ...
            <Cell :width="12">
              <FormItem label="课程安排">
                <Select v-model="video_id" :datas="videos[course_id]" key-name="id" title-name="title" :filterable="true" />
              </FormItem>
            </Cell>
            <Cell :width="6">
              <FormItem label="服务商">
                <Select v-model="service" :datas="services" key-name="id" title-name="title" />
              </FormItem>
            </Cell>
            <Cell :width="6">
              <FormItem>
                <p-button glass="h-btn h-btn-primary" permission="addons.Zhibo.zhibo.open" text="开始直播" @click="create()" />
              </FormItem>
            </Cell>
          </Row>
        </Form>
      ...
      <template v-if="push_url">
        <div class="float-box mb-10">
          <div class="h-input-group mb-10 w-400">
            ...
            <input v-model="push_url" type="text">
            <Button color="primary" @click="$Clipboard({ text: push_url })">复制</Button>
          </div>
          <div class="h-input-group mb-10 w-400">
            <span class="h-input-addon">OBS服务器</span>
            <input v-model="obs.server" type="text">
            <Button color="primary" @click="$Clipboard({ text: obs.server })">复制</Button>
          </div>
          <div class="h-input-group mb-10 w-400">
            <span class="h-input-addon">OBS串流密钥</span>
            <input v-model="obs.token" type="text">
            <Button color="primary" @click="$Clipboard({ text: obs.token })">复制</Button>
          </div>
        </div>
        <div class="float-box mb-10">
          <p-button glass="h-btn h-btn-primary" permission="addons.Zhibo.zhibo.pause" text="停止直播" @click="pause()" />

          <p-button glass="h-btn h-btn-primary" permission="addons.Zhibo.zhibo.resume" text="恢复直播" @click="resume()" />
        </div>
      </template>
    </div>
  </div>
