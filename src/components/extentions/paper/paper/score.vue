      ...
      <div class="float-box mb-10">
        <div v-for="item in questions" :key="item.id" class="question-item">
          <template v-if="item.question">
            ...
            <div class="content">
              <div class="mb-10" v-html="item.question.content" />
              <div v-if="item.question.type === 1 || item.question.type === 2" class="option">
            ...
            <div v-if="item.question.type !== 4" class="result">
              <p :class="item.is_correct ? 'green' : 'red'">{{ item.is_correct ? '正确' : '错误' }} | 本题得分：{{ item.score }}分</p>
            </div>
            <div v-if="item.question.remark" class="remark">
              <p>解析：</p>
              <p v-html="item.question.remark" />
            </div>
            <div v-if="userPaper.status === 3 && item.question.type === 4" class="score">
              <p>请打分：</p>
              <Select v-model="item.score" :datas="scoreList(item.question.score)" key-name="id" title-name="text" />
            </div>
          </template>
        </div>
      </div>
