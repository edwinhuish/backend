...
<template>
  <div class="h-panel w-1200">
    ...
    <div class="h-panel-body">
      <Row v-if="user" class="base-info mb-10">
        <Cell width="6">
          ID：
        ...
        <template v-if="user.profile">
          ...
          <Cell
            v-if="user.profile.diploma"
            width="6"
          >毕业证书：<br><img
            :src="user.profile.diploma"
            width="100"
            height="100"
            @click="$ImagePreview(user.profile.diploma)"
          ></Cell>
          <Cell
            v-if="user.profile.id_frontend_thumb"
            width="6"
          >身份证正面照：<br><img
          ...
          ></Cell>
          <Cell
            v-if="user.profile.id_backend_thumb"
            width="6"
          >身份证反面照：<br><img
          ...
          ></Cell>
          <Cell
            v-if="user.profile.id_hand_thumb"
            width="6"
          >手持身份证照：<br><img
            :src="user.profile.id_hand_thumb"
            width="100"
            height="100"
            @click="$ImagePreview(user.profile.id_hand_thumb)"
          ></Cell>
        </template>

        <Cell width="24" class="mt-10 mb-10">
          <p-button glass="h-btn h-btn-s h-btn-primary" permission="member.credit1.change" text="积分变动" @click="credit1Change()" />
        </Cell>
      </Row>

      <Row class="mb-10">
        <Cell width="24">
          <Tabs v-model="selectTab" :datas="tabs" @change="tabChange" />
        </Cell>
      </Row>
      ...
      <Row v-show="selectTab === 'collect'">
        <Cell width="24">
          ...
          <Pagination v-model="paginate.collect" align="right" @change="paginateChange('collect')" />
        </Cell>
      </Row>
      ...
      <Row v-show="selectTab === 'invite'">
        <Cell width="24">
          <Table :datas="userInvite" class="mb-10">
            ...
            <TableItem prop="nick_name" title="昵称" />
            <TableItem prop="mobile" title="手机号" />
            <TableItem title="过期时间" :width="120">
              <template slot-scope="{ data }">
                <date-text :date="data.invite_user_expired_at" />
              </template>
            </TableItem>
          ...
          <Pagination v-model="paginate.invite" align="right" @change="paginateChange('invite')" />
        </Cell>
      </Row>
      ...
      <Row v-show="selectTab === 'history'">
        <Cell width="24">
          ...
          <Pagination v-model="paginate.history" align="right" @change="paginateChange('history')" />
        </Cell>
      </Row>
      ...
      <Row v-show="selectTab === 'courses'">
        <Cell width="24">
          <Table :datas="userCourses" class="mb-10">
            ...
            <TableItem prop="charge" title="购买价格" unit="元" />
          ...
          <Pagination v-model="paginate.courses" align="right" @change="paginateChange('courses')" />
        </Cell>
      </Row>
      ...
      <Row v-show="selectTab === 'videos'">
        <Cell width="24">
          ...
          <Pagination v-model="paginate.videos" align="right" @change="paginateChange('videos')" />
        </Cell>
      </Row>
        ...
        <Cell width="24">
          <Table :datas="userRoles" class="mb-10">
            ...
            <TableItem title="开始" :width="120">
              <template slot-scope="{ data }">
                <date-text :date="data.started_at" />
              </template>
            </TableItem>
            <TableItem title="结束" :width="120">
              <template slot-scope="{ data }">
                <date-text :date="data.expired_at" />
              </template>
            </TableItem>
          </Table>
          <Pagination v-model="paginate.roles" align="right" @change="paginateChange('roles')" />
        </Cell>
      </Row>
      ...
      <Row v-show="selectTab === 'orders'">
        <Cell width="24">
          <Table :datas="userOrders" class="mb-10">
            <TableItem prop="id" title="ID" />
            <TableItem prop="order_id" title="订单号" />
            <TableItem prop="charge" title="价格" unit="元" />
            <TableItem title="状态">
              <template slot-scope="{ data }">
          ...
          <Pagination v-model="paginate.orders" align="right" @change="paginateChange('orders')" />
        </Cell>
      </Row>
      ...
      <Row v-show="selectTab === 'credit1Records'">
        <Cell width="24">
          ...
          <Pagination v-model="paginate.credit1Records" align="right" @change="paginateChange('credit1Records')" />
        </Cell>
      </Row>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getUserCredit1Records(reset = false) {
      ...
      const data = this.paginate.credit1Records
      data.id = this.id
    ...
    getUserCourses(reset = false) {
      ...
      const data = this.paginate.courses
      data.id = this.id
    ...
    getUserVideos(reset = false) {
      ...
      const data = this.paginate.videos
      data.id = this.id
    ...
    getUserRoles(reset = false) {
      ...
      const data = this.paginate.roles
      data.id = this.id
    ...
    getUserOrders(reset = false) {
      ...
      const data = this.paginate.orders
      data.id = this.id
    ...
    getUserCollect(reset = false) {
      ...
      const data = this.paginate.collect
      data.id = this.id
    ...
    getUserHistory(reset = false) {
      ...
      const data = this.paginate.history
      data.id = this.id
    ...
    getUserInvite(reset = false) {
      ...
      const data = this.paginate.invite
      data.id = this.id
