  ...
  <div class="table-basic-vue frame-page h-panel">
    ...
    <div class="h-panel-body">
      <div style="margin-bottom: 10px;">
        <Tabs v-model="tab" :datas="tabs" />
      </div>

      <div v-if="tab === 'local'">
        <Table :loading="loading" :datas="local">
          <TableItem prop="name" title="插件名" />
          <TableItem prop="version" title="当前版本" />
          <TableItem title="启用" align="center">
            <template slot-scope="{ data }">
              <h-switch v-model="data.enabled" :small="true" @change="switchHandler(data)" />
            </template>
          </TableItem>
      ...
      <div v-if="tab === 'repository'">
          ...
          <TableItem title="插件">
            <template slot-scope="{ data }">
              <a :href="'https://meedu.vip/addons/' + data.id + '/' + data.sign" target="_blank">
                {{ data.name }}
                <span v-if="data.is_buy" class="h-tag h-tag-red">已购买</span>
              </a>
            </template>
          </TableItem>
          <TableItem prop="version" title="版本" />
          <TableItem title="价格">
            <template slot-scope="{ data }">
        ...
        <div class="mt-10">
          <Pagination v-model="repositoryPaginate" align="right" @change="changePage" />
        </div>
      </div>
    </div>
  </div>
